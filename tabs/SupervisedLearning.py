...
def render(df: pd.DataFrame):
    ...
    with modelPredictionTab:
        ...
        # Cross validation to check best value for n, result was any value from 4 to 8, choosing 6
        # for n in range(1, 101):
        #     knn = KNeighborsRegressor(n_neighbors=n)
        #     scores = cross_val_score(knn, X_train, y_train, cv=5, scoring='r2')
        #     print(f"k={n}: {scores.mean():.2f}")
        ...
        with st.container(border=True):
            ...
            with knnEvalCol:
                ...
                st.markdown("###### Bad performance in both metrics. Cross validation was used to determine the best " \
                "value for neighbors and the performance is still bad. It seems like the current dataset does not " \
